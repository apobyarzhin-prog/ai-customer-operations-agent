...
type TriageResult = { priority: 'low' | 'normal' | 'high' | 'urgent'; recommended_status: 'open' | 'in_progress' | 'resolved'; summary: string; suggested_reply: string; confidence: number; reasoning: string[]; provider: 'openai' | 'demo' | 'demo_fallback' }
...
function App() {
  ...
  const triageProvider = (value: TriageResult['provider']) => locale === 'ru' ? ({ openai: 'OpenAI', demo: 'Demo', demo_fallback: 'Demo fallback' }[value]) : value === 'demo_fallback' ? 'Demo fallback' : value === 'openai' ? 'OpenAI' : 'Demo'
  const uiCopy = {
  ...
  if (authChecking) return <AuthScreen copy={t} locale={locale} dark={dark} onLocale={setLocale} onTheme={() => setDark(!dark)} loading />
  if (!session) return <AuthScreen copy={t} locale={locale} dark={dark} onLocale={setLocale} onTheme={() => setDark(!dark)} email={email} password={password} error={authError} onEmail={setEmail} onPassword={setPassword} onSubmit={(event) => void signIn(event)} />
...
function LanguageMenu({ locale, onLocale, label }: { locale: Locale; onLocale: (locale: Locale) => void; label: string }) {
  const [open, setOpen] = useState(false)
  const selected = localeOptions.find((option) => option.value === locale) ?? localeOptions[0]
  return <div className="language-menu"><button className="language-menu-trigger" type="button" aria-haspopup="menu" aria-expanded={open} aria-label={label} onClick={() => setOpen((value) => !value)} onKeyDown={(event) => { if (event.key === 'Escape') setOpen(false) }}><Languages size={15} aria-hidden="true" /><span>{selected.value.toUpperCase()} · {selected.label}</span><ChevronDown size={13} aria-hidden="true" /></button>{open && <div className="language-menu-popover" role="menu" aria-label={label}>{localeOptions.map((option) => <button className={`language-option ${option.value === locale ? 'selected' : ''}`} type="button" role="menuitem" key={option.value} onClick={() => { onLocale(option.value); setOpen(false) }}>{option.value.toUpperCase()} <span>{option.label}</span></button>)}</div>}</div>
}

function AuthScreen({ copy, locale, dark, onLocale, onTheme, email = '', password = '', error = false, loading = false, onEmail, onPassword, onSubmit }: { copy: Copy; locale: Locale; dark: boolean; onLocale: (locale: Locale) => void; onTheme: () => void; email?: string; password?: string; error?: boolean; loading?: boolean; onEmail?: (value: string) => void; onPassword?: (value: string) => void; onSubmit?: (event: FormEvent<HTMLFormElement>) => void }) {
  const authContent: Record<Locale, { kicker: string; headline: string; proofTitle: string; proofBody: string; themeLight: string; themeDark: string }> = {
    en: { kicker: 'CUSTOMER OPERATIONS', headline: 'Make every customer moment feel handled.', proofTitle: 'One calm place for the queue.', proofBody: 'Conversations, context and next steps together.', themeLight: 'Light', themeDark: 'Dark' },
    pl: { kicker: 'OBSŁUGA KLIENTA', headline: 'Spraw, by każda rozmowa z klientem była zaopiekowana.', proofTitle: 'Jedno spokojne miejsce dla kolejki.', proofBody: 'Rozmowy, kontekst i kolejne kroki w jednym miejscu.', themeLight: 'Jasny', themeDark: 'Ciemny' },
    de: { kicker: 'KUNDENOPERATIONS', headline: 'Jeder Kundenmoment fühlt sich gut betreut an.', proofTitle: 'Ein ruhiger Ort für die Warteschlange.', proofBody: 'Gespräche, Kontext und nächste Schritte zusammen.', themeLight: 'Hell', themeDark: 'Dunkel' },
    es: { kicker: 'OPERACIONES DE CLIENTES', headline: 'Haz que cada momento con el cliente se sienta atendido.', proofTitle: 'Un lugar tranquilo para la cola.', proofBody: 'Conversaciones, contexto y próximos pasos juntos.', themeLight: 'Claro', themeDark: 'Oscuro' },
    ru: { kicker: 'КЛИЕНТСКИЕ ОПЕРАЦИИ', headline: 'Каждое обращение — под контролем команды.', proofTitle: 'Одно спокойное место для всей очереди.', proofBody: 'Обращения, контекст и следующие шаги — вместе.', themeLight: 'Светлая', themeDark: 'Тёмная' },
  }
  const content = authContent[locale]
  return <main className="auth-shell"><div className="auth-topbar"><div className="auth-status"><span className="auth-status-dot" /> {copy.secureWorkspace ?? 'Secure workspace access'}</div><div className="auth-controls"><LanguageMenu locale={locale} onLocale={onLocale} label={copy.language ?? 'Language'} /><button className="auth-theme" type="button" onClick={onTheme} aria-label={copy.theme ?? 'Toggle theme'} aria-pressed={dark}>{dark ? <Sun size={16} /> : <Moon size={16} />}<span>{dark ? content.themeLight : content.themeDark}</span></button></div></div><div className="auth-layout"><section className="auth-intro" aria-label="Relay Operations"><div className="auth-brand"><BrandLogo src={PRODUCTION_LOGO_URL} name="Relay Operations" /><span><strong>Relay</strong><small>Operations</small></span></div><div className="auth-intro-copy"><span className="auth-kicker">{content.kicker}</span><h1>{content.headline}</h1><p>{copy.secureWorkspace ?? 'Secure workspace access for your operations team.'}</p></div><div className="auth-proof"><div className="auth-proof-icon"><ShieldCheck size={17} /></div><div><strong>{content.proofTitle}</strong><span>{content.proofBody}</span></div></div></section><section className="auth-card" aria-labelledby="auth-title"><div className="auth-card-heading"><div className="auth-icon"><KeyRound size={19} /></div><p className="eyebrow">{copy.workspace}</p><h2 id="auth-title">{copy.login}</h2><p className="auth-subtitle">{copy.secureWorkspace ?? 'Secure workspace access for your operations team.'}</p></div>{error && <p className="auth-error" role="alert"><ShieldCheck size={15} />{copy.invalidLogin}</p>}{loading ? <p className="auth-loading" role="status">{copy.sessionLoading}</p> : <form onSubmit={onSubmit} className="auth-form"><label htmlFor="auth-email"><span>{copy.email}</span><input id="auth-email" type="email" value={email} onChange={(event) => onEmail?.(event.target.value)} autoComplete="email" required /></label><label htmlFor="auth-password"><span>{copy.password}</span><input id="auth-password" type="password" value={password} onChange={(event) => onPassword?.(event.target.value)} autoComplete="current-password" required /></label><button className="button primary auth-submit" type="submit"><KeyRound size={16} /> {copy.signIn}</button></form>}<div className="auth-demo"><strong>{copy.demoCredentials}</strong><span>{DEMO_AUTH_ENABLED ? (copy.demoAuthEnabled ?? 'Demo auth is enabled for local development.') : (copy.demoAuthDisabled ?? 'Demo credentials work only when enabled by the backend.')}</span></div></section></div><p className="auth-footer">Relay Operations · {new Date().getFullYear()}</p></main>
}
